...
public class action extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("[money_back_action]收到的action是：" + action);
	...
	public void getRecord(HttpServletRequest request, HttpServletResponse response) throws SQLException, IOException, JSONException {
		...
		if (session.getAttribute("unit_db_name") == null) {
			...
			String sql="select * from users where member_email='"+user_email+"';";
			
			System.out.println("get_record里构造的sql="+sql);
			ResultSet rs = query_db.executeQuery(sql);

			if(rs.next()) {
				jsonObj.put("balance", rs.getString("balance"));
			}
			System.out.println("余额："+rs.getString("balance"));
			rs.close();
			query_db.close();
		}
//	/*
...
//		}
	...
	public void getMoney(HttpServletRequest request, HttpServletResponse response) throws SQLException, IOException, JSONException {
		HttpSession session = request.getSession();
		String action = request.getParameter("action");
		...
		if (session.getAttribute("unit_db_name") == null) {
			...
			if(rs.next()) {
				jsonObj.put("balance", rs.getString("user_balance"));
			}
			System.out.println("余额："+rs.getString("user_balance"));
			rs.close();
			query_db.close();
		}
