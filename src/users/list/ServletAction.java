...
public class ServletAction extends HttpServlet {
	...
	public String resultUrl="project/user_manage/"+resultPage;
	...
	public void queryRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		//这里可以修改成统一一个函数读取变量，下面的session里的attr可以用一个变量代替
		//检查输入参数是否正确先
		showDebug("[queryRecord]收到页面传过来的参数是：existResultset="+existResultset+",action="+action);
		/*----------------------------------------数据获取完毕，开始和数据库交互*/
		String creator=(String)session.getAttribute("user_name");
		String createTime=(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")).format(new Date());
		Users query=new Users();
	...
	public void addRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		System.out.println("[users.list/ServletAction/addRecord]收到的参数email="+user_email+",dbName="+dbName+",action="+action+",pwd="+user_pwd);
		JSONObject jsonObj=null;
		//检查输入参数是否正确先
		if(dbName!=null){
			String userId=session.getAttribute("user_id")==null?null:(String)session.getAttribute("user_id");
			...
			user.setMargin(user_margin);
			user.setCity(user_city);
			System.out.println("[Users/user对象内部分参数]-"+user.getAction()+",name="+user_name+",user_role="+user_role+",user_balance="+user_balance);
		...
		if(isAjax){
			...
			redirectUrl="../../project/user_manage/record_list.jsp";
			resultMsg=java.net.URLEncoder.encode(resultMsg, "UTF-8");
			String url = resultUrl+"?result_msg="+resultMsg+"&result_code="+resultCode+"&redirect_url="+redirectUrl;
	...
	public void modifyRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(id!=null && dbName!=null){
			...
			todo.setBalance(user_balance);
			todo.setMargin(user_margin);
			todo.setCity(user_city);
			todo.setName(user_name);
			todo.setPwd(user_pwd);
	...
	public void modifyUserInfo(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(id!=null && dbName!=null){
			...
			jsonObj=todoDao.modifyUserInfo(todo,request,response);
			ylxLog.log("用户 "+creator+" 于 "+createTime+" 修改了 ["+module+"]["+sub+"] 记录","修改记录",module);
		}else{
	...
	public void exportRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String exportFilePathName="C:\\upload\\project\\export\\XM09_users_export_"+(new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())+".xls";
	...
	public void exportData(JSONObject jsonObj,String filePathName) throws Exception {
		if(jsonObj!=null){
			...
			String[] cols={"index⊙text⊙id","time_interval⊙text⊙id","count⊙text⊙邮箱","color⊙text⊙密码","color_name⊙text⊙用户名","color_name⊙text⊙用户权限","color_name⊙text⊙创建时间","color_name⊙text⊙用户余额","color_name⊙text⊙用户预付款","color_name⊙text⊙用户所在地"};	//name⊙type⊙nick
			ew.CreExcel(jsonObj, "用户统计", cols, filePathName);
			jsonObj.put("result_code",0);
