...
import bonds_manage.list.Users;
import bonds_manage.list.UsersDao;
...
public class ServletAction extends HttpServlet {
	...
	//这里是需要改的,module和sub
	public String module = "bonds_manage";
	...
	public String resultUrl="project/bonds_manage/"+resultPage;
	...
	public void getRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		bonds_manage.list.Users query=new bonds_manage.list.Users();
		...
		if(existResultset.equals("1")){
			...
			//如果是新查询
			bonds_manage.list.UsersDao usersDao=new bonds_manage.list.UsersDao();
	...
	public void addRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		System.out.println("[bonds_manage.list/ServletAction/addRecord]收到的参数city="+city+",temp="+temp+",text="+text+",wind_dir="+wind_dir+",wind_scale="+wind_scale+",wind_speed="+wind_speed+",vis="+vis);
		JSONObject jsonObj=null;
		//检查输入参数是否正确先
		if(dbName!=null){
			String userId=session.getAttribute("user_id")==null?null:(String)session.getAttribute("user_id");
			...
			bonds_manage.list.Users user=new bonds_manage.list.Users();
			...
			user.setVis(vis);
			System.out.println("[bonds_manage对象内部分参数]-"+user.getAction()+",city="+city+",temp="+temp+",text="+text+",wind_dir="+wind_dir+",wind_scale="+wind_scale+",wind_speed="+wind_speed+",vis="+vis);
		...
		if(isAjax){
			...
			redirectUrl="../../project/bonds_manage/record_list.jsp";
			resultMsg=java.net.URLEncoder.encode(resultMsg, "UTF-8");
			String url = resultUrl+"?result_msg="+resultMsg+"&result_code="+resultCode+"&redirect_url="+redirectUrl;
	...
	public void exportData(JSONObject jsonObj,String filePathName) throws Exception {
		if(jsonObj!=null){
			...
			showDebug("导出开始！");
			String[] cols={"time_interval⊙text⊙ID","count⊙text⊙最新变动时间","color⊙text⊙期货指数","color_name⊙text⊙指数名称",
					"color_name⊙text⊙持有数量","color_name⊙text⊙闭仓价格"};	//name⊙type⊙nick
			ew.CreExcel(jsonObj, "持仓信息", cols, filePathName);
			jsonObj.put("result_code",0);
