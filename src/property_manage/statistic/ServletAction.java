package property_manage.statistic;
/*
 * 增删改查看导印统功能的实现
...
import weather.dao.Statistic;
import weather.dao.StatisticDao;
import weather.dao.Todo;
import weather.dao.TodoDao;
...
public class ServletAction extends HttpServlet {
	...
	public void statisticRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(dbName!=null){
			...
			property_manage.dao.StatisticDao statisticDao=new property_manage.dao.StatisticDao();
			
			//注意这里
			property_manage.dao.Statistic statistic=new property_manage.dao.Statistic();
		...
		if(isAjax){
			...
			resultUrl="project/property_manage/todo_statistic_result.jsp";
			showDebug("resultUrl="+resultUrl);
	...
	public String getStatisticGraph(HttpSession session,String dbName,JSONObject json,String title,String column,String statisticType,int statisticImage,int chartWidth,int chartHeight) throws SQLException, IOException, JSONException{
		...
		for(int i=0;i<arr.length();i++) {
			...
			if(statisticType.equals("no")){
				...
				System.out.println("出现了，要注意zzzzzzzz");
			}
