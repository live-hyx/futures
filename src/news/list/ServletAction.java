...
import news.list.Users;
import news.list.UsersDao;
...
public class ServletAction extends HttpServlet {
	...
	public String resultUrl="project/news/"+resultPage;
	...
	public void processAction(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		if(session.getAttribute("user_role")==null){
			...
			if (action == null){
				...
				if (action.equals("get_urlrecord")) {
					actionOk=true;
					try {
						getURLRecord(request, response);
					} catch (SQLException e) {
						e.printStackTrace();
					} catch (JSONException e) {
						e.printStackTrace();
	...
	public void getRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		news.list.Users query=new news.list.Users();
		...
		if(existResultset.equals("1")){
			...
			//如果是新查询
			news.list.UsersDao usersDao=new news.list.UsersDao();
	...
	public void getURLRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
		HttpSession session = request.getSession();
		String dbName=(String)session.getAttribute("unit_db_name");
		...
		//这里可以修改成统一一个函数读取变量，下面的session里的attr可以用一个变量代替
		//检查输入参数是否正确先
		showDebug("[getURLRecord]收到页面传过来的参数是："+existResultset+","+action+","+id);
		//----------------------------------------数据获取完毕，开始和数据库交互
		String creator=(String)session.getAttribute("user_name");
		String createTime=(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")).format(new Date());
		news.list.Users query=new news.list.Users();
		...
		if(existResultset.equals("1")){
			...
			//如果是新查询
			news.list.UsersDao usersDao=new news.list.UsersDao();
			jsonObj=usersDao.getURLRecord(query);
			session.setAttribute(module+"_"+sub+"_get_record_result",jsonObj);
		}
		jsonObj.put("user_id",userId);
		jsonObj.put("user_name",userName);
		...
		if(isAjax){
			...
			resultMsg=java.net.URLEncoder.encode(resultMsg, "UTF-8");
			String url = redirectPath+"/"+redirectUrl;
			showDebug("[jumpRecord]结果URL："+url);
			response.sendRedirect(url);
		}
	}
	...
	public void queryRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		//这里可以修改成统一一个函数读取变量，下面的session里的attr可以用一个变量代替
		//检查输入参数是否正确先
		showDebug("[queryRecord]收到页面传过来的参数是：existResultset="+existResultset);
		/*----------------------------------------数据获取完毕，开始和数据库交互*/
		//String creator=(String)session.getAttribute("user_name");
		//String createTime=(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")).format(new Date());
		news.list.Users query=new news.list.Users();
		...
		query.setName(user_name);
		query.setRole(userRole);
		query.setCreatetime(create_time);
		query.setTitle(title);
		query.setDescreption(description);
		query.setPicurl(pic_url);
		query.setUrl(url1);
		

		System.out.println("queryRecord/query 对象的name="+query.getName()+",action="+query.getAction()+",DbName="+query.getDbName());
		JSONObject jsonObj=null;
	...
	public void getRecordView(HttpServletRequest request, HttpServletResponse response) throws Throwable {
		...
		String url1=request.getParameter("url");
		//检查输入参数是否正确先
		showDebug("收到页面传过来的参数是：exist_resultset="+existResultset+",action="+action+",id="+id);
		/*----------------------------------------数据获取完毕，开始和数据库交互*/
		JSONObject jsonObj=null;
		news.list.Users query=new news.list.Users();
		query.setId(id);
		...
		if(existResultset.equals("1")){			//如果是新查询
			...
			if(session.getAttribute(module+"_"+sub+"_get_record_result")!=null){
				...
				jsonObj.put("pic_url",pic_url);
				jsonObj.put("url",url1);
				//然后还有导航信息
	...
	public void addRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String title=request.getParameter("title");
		String description=request.getParameter("description");
		String pic_url=request.getParameter("picurl");
		...
		if(dbName!=null){
			...
			/*----------------------------------------数据获取完毕，开始和数据库交互*/
			news.list.UsersDao usersDao=new news.list.UsersDao();
			news.list.Users user=new news.list.Users();
			user.setAction(action);
			user.setDbName(dbName);
			user.setCreatetime(createtime);
			user.setTitle(title);
			user.setDescreption(description);
			user.setPicurl(pic_url);
			user.setUrl(url);
			
			
			System.out.println("[News对象内部分参数]-"+user.getAction()+",createnumber="+createtime+",title="+title+",description="+description+",pic_url="+pic_url+",url="+url);
		...
		if(isAjax){
			...
			redirectUrl="../../project/news/record_list.jsp";
			resultMsg=java.net.URLEncoder.encode(resultMsg, "UTF-8");
			String url1 = resultUrl+"?result_msg="+resultMsg+"&result_code="+resultCode+"&redirect_url="+redirectUrl;
			showDebug("[addRecord]结果URL："+url1);
	...
	public void modifyRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(id!=null && dbName!=null){
			...
			todo.setTitle(title);
			todo.setDescreption(description);
			todo.setPicurl(pic_url);
			todo.setUrl(url1);
	...
	public void exportRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String exportFilePathName="C:\\upload\\project\\export\\XM09_news_export_"+(new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())+".xls";
		...
		try{
            ...
            String resultFileName = exportFilePathName;
            response.setCharacterEncoding("UTF-8");  
            response.setHeader("Content-disposition", "attachment; filename=" + resultFileName);// 设定输出文件头
            response.setContentType("application/msexcel");// 定义输出类型
            //输入流：本地文件路径
            DataInputStream in = new DataInputStream( new FileInputStream(new File(exportFilePathName)));  
            //输出流
	...
	public void exportData(JSONObject jsonObj,String filePathName) throws Exception {
		if(jsonObj!=null){
			...
			String[] cols={"time_interval⊙text⊙ID","time_interval⊙text⊙标题","count⊙text⊙来源","color⊙text⊙发布时间","color_name⊙text⊙封面图片链接","color_name⊙text⊙新闻网址链接"};	//name⊙type⊙nick
			ew.CreExcel(jsonObj, "新闻信息", cols, filePathName);
			jsonObj.put("result_code",0);
