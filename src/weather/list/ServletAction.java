...
import weather.list.Users;
import weather.list.UsersDao;
...
public class ServletAction extends HttpServlet {
	...
	public String resultUrl="project/weather/"+resultPage;
	...
	public void queryRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		//这里可以修改成统一一个函数读取变量，下面的session里的attr可以用一个变量代替
		//检查输入参数是否正确先
		showDebug("[queryRecord]收到页面传过来的参数是：existResultset="+existResultset+",city="+city);
		/*----------------------------------------数据获取完毕，开始和数据库交互*/
		String creator=(String)session.getAttribute("user_name");
		String createTime=(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")).format(new Date());
		weather.list.Users query=new weather.list.Users();
	...
	public void addRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String wind_dir = request.getParameter("wind_dir");
		String wind_scale = request.getParameter("wind_scale");
		String wind_speed = request.getParameter("wind_speed");
		String vis = request.getParameter("vis");
		System.out.println("[weather.list/ServletAction/addRecord]收到的参数city="+city+",temp="+temp+",text="+text+",wind_dir="+wind_dir+",wind_scale="+wind_scale+",wind_speed="+wind_speed+",vis="+vis);
		JSONObject jsonObj=null;
		//检查输入参数是否正确先
		if(dbName!=null){
			String userId=session.getAttribute("user_id")==null?null:(String)session.getAttribute("user_id");
			...
			weather.list.Users user=new weather.list.Users();
	...
	public void exportRecord(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String exportFilePathName="C:\\upload\\project\\export\\XM09_weather_export_"+(new SimpleDateFormat("yyyyMMddHHmmss")).format(new Date())+".xls";
	...
	public void exportData(JSONObject jsonObj,String filePathName) throws Exception {
		if(jsonObj!=null){
			...
			String[] cols={"time_interval⊙text⊙ID","time_interval⊙text⊙城市","count⊙text⊙数据更新时间","color⊙text⊙温度","color_name⊙text⊙天气代码","color_name⊙text⊙天气","color_name⊙text⊙风向","color_name⊙text⊙风力（级）","color_name⊙text⊙风速（米/秒）","color_name⊙text⊙能见度（米）"};	//name⊙type⊙nick
			ew.CreExcel(jsonObj, "天气信息", cols, filePathName);
			jsonObj.put("result_code",0);
