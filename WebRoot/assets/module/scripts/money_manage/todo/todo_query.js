...
var Page = function() {
	...
	function showQueryResult(json){
		...
		if(id != undefined){
			var temp=json.temp;
			var obstime=json.obs_time;
			var text=json.text;
			var wind_dir = json.wind_dir;
			var wind_scale = json.wind_scale;
			var wind_speed = json.wind_speed;
			var vis = json.vis;
			var createTime=json.create_time;
			html=html+"														<div style=\"clear:both;width:100%;margin-top:5px;border:0px solid blue;\">";
			html=html+"															<div style=\"display:table-cell;margin-left:10px;margin-right:10px;margin-top:10px;margin-bottom:10px;border:0px solid blue;\"><p>";
			html=html+"																<span>[id："+id+"][温度："+temp+"][数据更新时间："+obstime+"][天气："+text+"]";
			html=html+"[风向："+wind_dir+"][风力："+wind_scale+"级][风速："+wind_speed+"米/秒][能见度："+vis+"米]</span><p>";
			html=html+"															</div>";
			html=html+"														</div>";
	...
	var checkInput=function(){
		var bOk=true;
		var city=$("#city").val();
		if(city==null || city==""){Frame.showMsg("查询城市名名不能为空！");bOk=false;};
		return bOk;
	};
