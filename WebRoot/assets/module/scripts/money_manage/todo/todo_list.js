...
var Page = function() {
	...
	var showResult=function(json){
		...
		if(json!=null){
			...
			var tip="当前查询到了 "+list.length+" 条账单信息";
	...
	var showRecordList=function(list){
		html="<div class=\"icon-btn\"><i class=\"fa fa-bar-chart-o\"><i>历史充值信息</i></i></div>"
	...
	var showRecord = function(json){
		...
		var amount=json[2];
		var op_time=json[3];
		//todo_list.js:187 1 充值 1000.00 2020-12-23 01:14:39.0 
		...
		html=html+"																 <i class=\"fa fa-money\"></i>";
		console.log(op_type);
		if(op_type=="充值") {
			console.log("1");
			html=html+"		                                                         <span class=\"label label-sm label-success\">[操作类型:"+op_type+"][额度："+amount+"][操作时间："+op_time+"]</span>";
		}
		else{
			console.log("2");
			html=html+"		                                                         <span class=\"label label-sm label-warning\">[操作类型:"+op_type+"][额度："+amount+"][操作时间："+op_time+"]</span>";
		} 
		html=html+"																<button  type=\"button\" class=\"btn green btn-xs\" onclick=\"Page.deleteRecord("+id+");\">删除</button>";	
		html=html+"															</div>";
		html=html+"														</div>";
	};
	var help=function(){
