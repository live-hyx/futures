...
var Page = function() {
	...
	var showRecord = function(json){
		...
		var text = json[5];
		var winddir = json[6];
		var windscale= json[7];
		var windspeed= json[8];
		var vis=json[9];
		$("#user_id").val(id);
		$("#city").val(city);
		$("#obstime").val(obstime);
	...
	var submitRecord=function(){
		if(checkInput()==true){
			...
			var city=$("#city").val();
			var obstime=$("#obstime").val();
			var temp=$("#temp").val();
			var icon=$("#icon").val();
			var text=$("#text").val();
			var winddir=$("#winddir").val();
			var windscale=$("#windscale").val();
			var windspeed=$("#windspeed").val();
			alert(winddir+"------"+windscale+"------"+windspeed)
			var vis=$("#vis").val();
			page_form.action="../../"+module+"_"+sub+"_servlet_action?action=modify_record&user_id="+user_id
			+"&city="+name+"&obstime="+obstime+"&temp="+temp+"&icon="+icon
			+"&text="+text+"&winddir="+winddir+"&windscale="+windscale+"&windspeed="+windspeed
			+"&vis="+vis;
