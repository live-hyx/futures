...
var Record = function() {
	...
	var sortRecord=function(index,sortName){
		...
		var url="../../"+module+"_"+sub+"_servlet_action?" +
				"action=get_record&id="+id+"&sort_index"+index+"&order_by="+sortName;
		console.log(url);
		$.post(url,function(json){
...
var Page = function() {
	...
	var showResult=function(json){
		...
		if(json!=null){
			...
			var tip="当前查询到了 "+list.length+" 条交易信息";
	...
	var showRecord = function(json){
		//["10", "2021-01-12 09:11:19.0", "4", "5", "6", "7.00", "0", 0]
		var id=json[0];
		var create_time=json[1];
		var future_index=json[2];
		var index_name=json[3];
		var quantity=json[4];
		var closing_price=json[5];
		var href="list_add_div.jsp?closing_price="+closing_price+"&future_index="+future_index+"&index_name="+index_name;
		console.log(href);
		console.log(json);
		html=html+"<div class=\"col-md-6 col-sm-6\"><div class=\"portlet blue-hoki box\"><div class=\"portlet-title\"><div class=\"caption\">"
		html=html+"<i class=\"fa fa-cogs\"></i>"+index_name+"("+future_index+")"+"</div><div class=\"actions\"><a href="+href+" class=\"btn btn-default btn-sm\" data-target=\"#ajax\" data-toggle=\"modal\"><i class=\"fa fa-pencil\"></i> 开仓 </a></div></div><div class=\"portlet-body\">";
		html=html+"<div class=\"row static-info\"><div class=\"col-md-5 name\">"+"持有数量:"+"</div><div class=\"col-md-7 value\">"+quantity+"</div></div>";
		html=html+"<div class=\"row static-info\"><div class=\"col-md-5 name\">"+"单价:"+"</div><div class=\"col-md-7 value\">"+closing_price+"</div></div>";
		html=html+"</div></div></div>"
		
		//html=html+"													<div><span id=\"tip_div\"></span>";
		...
		//html=html+"		                                                         <span class=\"label label-sm label-success\">[期货索引："+future_index+"][期货名称："+index_name+"][数量：" +
		//		                                                                     ""+quantity+"][开仓价格："+opening_price+"][创建时间："+create_time+"]</span>";
		//html=html+"																<button type=\"button\" class=\"btn green btn-xs\">开仓</button><p>";
